...
df_scale = function(x, center = T, scale = T) {
  ...
  var_classes = sapply(x, class)
  
  x_scaled = lapply(seq_along(x), function(i) {
    if (var_classes[i] == "numeric") {
      return(scale(x[[i]], center = center, scale = scale))
    } else {
      return(x[[i]])
    }
  })
  
  result = as_tibble(x_scaled)
...
#' Calculate the accuracy of binary classifier.
#'
...
#' @param sig.level The desired significance level (default is 0.05).
#'
...
minimum_n_per_group = function(d, power = 0.8, sig.level = 0.05) {
  if (!is.numeric(d) || length(d) != 1) {
    stop("Expected Cohen's d should be a single numeric value.")
  }
  
  if (!is.numeric(power) || length(power) != 1) {
    stop("Desired statistical power should be a single numeric value.")
  }
  
  if (!is.numeric(sig.level) || length(sig.level) != 1) {
    stop("Desired significance level should be a single numeric value.")
  }
  
  # Calculate critical values for the desired power and significance level
  critical_value_alpha = qnorm(1 - sig.level / 2)
  critical_value_power = qnorm(power)
  
  # Calculate the necessary sample size
  n = (critical_value_alpha + critical_value_power)^2 / d^2
